				...
				if ($this->order_has_week_end($order)) {
					...
					if ( ! empty($response->error)) {
						...
						if ($this->is_no_such_customer_error($response->error)) {
							if (WC_Stripe_Helper::is_pre_30()) {
								delete_user_meta($order->customer_user,
									'_stripe_customer_id');
								delete_post_meta($order_id,
									'_stripe_customer_id');
							} else {
								delete_user_meta($order->get_customer_id(),
									'_stripe_customer_id');
						...
						if ($this->is_no_such_token_error($response->error) && $prepared_source->token_id) {
							...
							$wc_token->delete();
							$localized_message = __('This card is no longer available and has been removed.',
								'woocommerce-gateway-stripe');
							$order->add_order_note($localized_message);
							throw new WC_Stripe_Exception(print_r($response,
								true), $localized_message);
						}
						...
						if ($this->is_retryable_error($response->error)) {
							if ($retry) {
								...
								return $this->process_payment($order_id, true,
									$force_save_source, $response->error);
							} else {
					...
					do_action('wc_gateway_stripe_process_payment', $response,
						$order);
