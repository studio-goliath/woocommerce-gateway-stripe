			...
			if ( $charge && 'no' === $captured ) {
				...
				if ($customer_id && 'no' == $customer_charge_captured) {
					...
					if (is_wp_error($result)) {
						$order->add_order_note(__('Unable to capture charge!',
								'woocommerce-gateway-stripe') . ' ' . $result->get_error_message());
					} else {
						$order->add_order_note(sprintf(__('Stripe charge complete (Charge ID: %s)',
							'woocommerce-gateway-stripe'), $result->id));
						update_post_meta($order_id,
							'_stripe_customer_charge_captured', 'yes');
						delete_post_meta($order_id, '_stripe_customer_id');

						// Store other data such as fees
						update_post_meta($order_id, 'Stripe Payment ID',
							$result->id);
